...
public class MissingFactExtractor extends Extractor {
  ...
  public Set<Theme> input() {
    return new FinalSet<>(FactExtractor.YAGOFACTS, LiteralFactExtractor.YAGOLITERALFACTS, LabelExtractor.YAGOLABELS, MetaFactExtractor.YAGOMETAFACTS);
  }

  /** Comparison theme*/
  public static final Theme MISSING_FACTS=new Theme("missingFacts","Facts that were there in the previous version of YAGO and that are no longer there.");
  ...
  public void extract(Map<Theme, FactWriter> output, Map<Theme, FactSource> input) throws Exception {
    for(Theme checkMe : input.keySet()) {
      ...
      output.get(MISSING_FACTS).write(new Fact(FactComponent.forTheme(checkMe),"rdf:type","<follows>"));
      for(Fact f : old) {
        output.get(MISSING_FACTS).write(f);
      }
      Announce.done();
    }    
  }
