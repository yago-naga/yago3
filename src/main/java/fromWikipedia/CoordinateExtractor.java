...
public class CoordinateExtractor extends EnglishWikipediaExtractor {
  ...
  protected void writeCoords(String entity, Double la, Double lo) throws IOException {
    if (la != null) write(COORDINATES, new Fact(entity, "<hasLatitude>", FactComponent.forDegree(la)), COORDINATE_SOURCES,
        FactComponent.wikipediaURL(entity), "CoordinateExtractor");
    if (lo != null) write(COORDINATES, new Fact(entity, "<hasLongitude>", FactComponent.forDegree(lo)), COORDINATE_SOURCES,
        FactComponent.wikipediaURL(entity), "CoordinateExtractor");
  ...
  public static Double getCoord(String degree, String minute, String second, String eastWestNorthSouth) {
    int sign = 1;
    if (!eastWestNorthSouth.isEmpty() && "SswW".indexOf(eastWestNorthSouth.charAt(0)) != -1) sign = -1;
    try {
    ...
    while (true) {
      switch (FileLines.findIgnoreCase(in, "<title>", "{{coord", "{{ coord")) {
        ...
        case 2:
          ...
          if (m.find()) {
            writeCoords(titleEntity, getCoord(m.group(1), m.group(2), "0", m.group(3)), getCoord(m.group(4), m.group(5), "0", m.group(6)));
            break;
          }
  ...
  public static void main(String[] args) throws Exception {
    ...
    new CoordinateExtractor(new File("/san/jbiega/multiWikiSymlinks/en_wiki-20121201-pages-articles.xml")).extract(new File("/san/suchanek/yago3"),
        "Single run.");
  }
