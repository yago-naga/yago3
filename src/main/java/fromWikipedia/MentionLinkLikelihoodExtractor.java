...
public class MentionLinkLikelihoodExtractor extends MultilingualWikipediaExtractor {
  ...
  private void loadMentions() throws IOException {	  
	  ...
	  fs.addAll(RedirectExtractor.REDIRECTFACTS.inLanguage(language).factCollection().getFactsWithRelation("<redirectedFrom>"));
	  
	  fs.addAll(StructureExtractor.STRUCTUREFACTS.inLanguage(language).factCollection().getFactsWithRelation("<hasAnchorText>"));
