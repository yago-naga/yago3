  ...
  public void extract() throws Exception {
    ...
    while (nr.hasNext()) {
      ...
      // Format of regular expression is taken from: https://www.wikidata.org/wiki/Property:P18
        ...
        if (!images.containsKey(f.getRelation())) images.put(f.getRelation(), f.getObject());
        prevImage = f;
      }
      // If the rank of the image was "PreferredRank", replace the existing image with this preferred image.
        ...
        if (f.getObject().endsWith("#PreferredRank>")) images.put(prevImage.getRelation(), prevImage.getObject());
        prevImage = null;
      }
    }
    nr.close();
