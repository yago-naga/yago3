...
public class AttributeMatcher extends Extractor {
  ...
  public Set<Theme> input() {
    HashSet<Theme> result = new HashSet<Theme>(Arrays.asList(InfoboxMapper.INFOBOXFACTS_MAP.get("en"), InterLanguageLinks.INTERLANGUAGELINKS,
        PatternHardExtractor.INFOBOXPATTERNS, HardExtractor.HARDWIREDFACTS, WordnetExtractor.WORDNETWORDS,
  ...
  public void extract(Map<Theme, FactWriter> writers, Map<Theme, FactSource> input) throws Exception {
    ...
    for (Fact f2 : lang2FactSource) {
      ...
      for (Fact f : yagoFactsWithSubject) {
        ...
        } else deduce(yagoRelation, secondLangRelation, false);
      }
      ...
      for (Fact f : yagoFactsWithObject) {
        ...
        if (isEntity(expectedDatatype)) {
          if (yagoSubject.equals(secondLangObject)/*, "<yagoGeoEntity>", preferredMeaning && isEqualr(yagoObject, secondLangSubject)*/) deduce(
              yagoRelation, "<" + FactComponent.stripBrackets(secondLangRelation) + "->", true);
          else deduce(yagoRelation, "<" + FactComponent.stripBrackets(secondLangRelation) + "->", false);
        }
      }
    }
  ...
  public double numberSim(String n1, String n2) {
    ...
    else result = (double) (0.75 * Math.min(num1, num2) / Math.max(num1, num2));
    return result;
  }
  ...
  public double numberSetSim(List<String> s1, String s2) {
    ...
    double sum = 0;
    for (String str : s1) {
      sum += numberSim(FactComponent.stripQuotes(FactComponent.getString(str)), FactComponent.stripQuotes(FactComponent.getString(s2)));
    }
  ...
  public double dateSim(String s1, String s2) {
    ...
    float sum = 0;
    if (isNumeric(date1[0]) && isNumeric(date2[0])) sum = (Integer.parseInt(date1[0]) - Integer.parseInt(date2[0])) * 365;
    if (isNumeric(date1[1]) && isNumeric(date2[1])) sum += (Integer.parseInt(date1[1]) - Integer.parseInt(date2[1])) * 30;
    if (isNumeric(date1[2]) && isNumeric(date2[2])) sum += (Integer.parseInt(date1[1]) - Integer.parseInt(date2[1]));
    if (sum < 366) return 1;
  ...
  public static Set<String> numberParser(String input) {
    ...
    for (int i = 0; i < input.length(); i++) {
      if (isNumeric(input.substring(i, i + 1))) {
        ...
        if ((Pattern.matches("[0-9]+[.,][0-9]+", result))) numberPortions.add(result);
        result = "";
      }
    }
    if (Pattern.matches("[0-9]+[.,][1-9]+", result)) numberPortions.add(result);
  ...
  public static String preprocess(String input) throws IOException {
    ...
    for (int i = 1; i < input.length() - 1; i++)
      if ((input.charAt(i) == '.') && Character.isDigit(input.charAt(i - 1)) && Character.isDigit(input.charAt(i + 1))) input = input
          .replace(".", "");

    for (int i = 1; i < input.length() - 1; i++)
      if ((input.charAt(i) == ',') && Character.isDigit(input.charAt(i - 1)) && Character.isDigit(input.charAt(i + 1))) input = input.replace(",",
          ".");
