...
public class CategoryTypeExtractor extends Extractor {
	...
	public static final Theme CATEGORYTYPES = new Theme("categoryTypes",
			"All rdf:type facts of YAGO", ThemeGroup.TAXONOMY);
	...
	public Set<Theme> output() {
		return new FinalSet<Theme>(CATEGORYTYPESOURCES.inLanguage(language),
				...
				CATEGORYCLASSES.inLanguage(language));
	}
	 ...
	 * @param classWriter
	 */
	protected void extractType(String titleEntity, String category,
			Set<String> types) throws IOException {
		String concept = category2class(category);
		if (concept == null)
			return;
		types.add(FactComponent.forWikiCategory(category));
		categoryClassFacts.add(
				new Fact(null, FactComponent.forWikiCategory(category),
						RDFS.subclassOf, concept), FactComponent
						.wikipediaURL(titleEntity),
	...
	public void extract(Map<Theme, FactWriter> writers,
		...
		if (this.language.equals("en"))
			categoryMembs = ExtendedFactCollection
					.loadFacts(input.get(CategoryExtractor.CATEGORYMEMBERS
							.inLanguage(language)));
		else
			categoryMembs = ExtendedFactCollection.loadFacts(input
					.get(CategoryExtractor.CATEGORYMEMBERS_TRANSLATED
							.inLanguage(language)));

		nonConceptualCategories = new FactCollection(
				input.get(PatternHardExtractor.CATEGORYPATTERNS))
				.asStringSet("<_yagoNonConceptualWord>");
		preferredMeanings = WordnetExtractor.preferredMeanings(input);
		wordnetClasses = new FactCollection(
				input.get(WordnetExtractor.WORDNETCLASSES), true);
		...
		for (Fact f : categoryClassFacts) {
			if (FactComponent.isFactId(f.getArg(1)))
				writers.get(CATEGORYTYPESOURCES.inLanguage(language)).write(f);
			else
				writers.get(CATEGORYCLASSES.inLanguage(language)).write(f);
		}
		Announce.done();
	...
	/** Writes the facts */
	public void flush(String entity, Set<String> types,
		...
		for (String type : types) {
			write(writers, CATEGORYTYPES.inLanguage(language), new Fact(entity,
					RDFS.type, type), CATEGORYTYPESOURCES.inLanguage(language),
