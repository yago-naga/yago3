...
public class Theme extends FactSource.FileFactSource implements
	...
	public static synchronized Theme getOrCreate(String name,
			String description, ThemeGroup group) {
	...
	public synchronized void openForWritingInFolder(File folder, String header)
		...
		if (file != null)
			throw new RuntimeException("Theme " + this + " already written to "
					+ file);
	...
	public synchronized Theme assignToFolder(File folder) throws IOException {
		...
		if (file != null) {
			if (file.equals(f))
				...
				throw new IOException("Theme " + this
						+ " is already assigned to file " + file
						+ ", cannot assign it to " + f);
		}
		file = f;
		cache = null;
		return (this);
