...
//		"C://Users//Administrator//Downloads//turtle-20130808-links.ttl"
		...
		while(nr.hasNext()){
			...
			if(f.getArg(2).contains("#Item")){
				String englishName= correspondence.get("en");
				correspondence.remove("en");
				if(englishName !=null){
					englishName = Char.cutLast(englishName);
					for (Map.Entry<String, String> entry : correspondence.entrySet())
					{
						writer.write(new Fact(FactComponent.forYagoEntity(Char.decodePercentage(englishName)),  "rdfs:label"
	...
	public static void main(String[] args) {
		try {
			new InterLanguageLinks(new File("C://Users//Administrator//data//wikidata.rdf"))
			.extract(new File("C:/Users/Administrator/data2/yago2s"), "test");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
